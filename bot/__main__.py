...
<br><br>
<b>/{BotCommands.ShellCommand}</b>: Run commands in Shell (Only Owner & Sudo)
<br><br>
<b>/{BotCommands.ExecHelpCommand}</b>: Get help for Executor module (Only Owner)
<br><br>
<b>/{BotCommands.AddSudoCommand}</b>: Add sudo user (Only Owner & Sudo)
<br><br>
<b>/{BotCommands.RmSudoCommand}</b>: Remove sudo users (Only Owner & Sudo)
<br><br>
...
if SET_BOT_COMMANDS:
     botcmds = [
        ...
        (f'{BotCommands.SpeedCommand}','Testspeed'),
...
def main():
        ...
        if notifier_dict:
            for cid, data in notifier_dict.items():
                if ospath.isfile(".restartmsg"):
                    ...
                    msg = '🥱I AM AGAIN ALIVE😎🔥❗'
                else:
                    msg = '🥱I AM START AGAIN NOW🔥❗'
                for tag, links in data.items():
                     msg += f"\n\n{tag}: "
                     for index, link in enumerate(links, start=1):
                         msg += f" <a href='{link}'>{index}</a> |"
                         if len(msg.encode()) > 4000:
                             if '🥱I AM AGAIN ALIVE😎🔥❗' in msg and cid == chat_id:
    ...
    if ospath.isfile(".restartmsg"):
        with open(".restartmsg") as f:
            chat_id, msg_id = map(int, f)
        bot.edit_message_text("🥱I AM AGAIN ALIVE😎🔥❗", chat_id, msg_id)
        osremove(".restartmsg")
